...
public class OLD_MovementAStar : MonoBehaviour
{
    // Player object
    ...
    void Start()
    {
        ...
        gridX = floor.GetComponentInChildren<Tilemap>().cellBounds.max.x + Mathf.Abs(floor.GetComponentInChildren<Tilemap>().cellBounds.min.x);
        gridY = floor.GetComponentInChildren<Tilemap>().cellBounds.max.y + Mathf.Abs(floor.GetComponentInChildren<Tilemap>().cellBounds.min.y);
        //Debug.Log("Grid X: " + gridX);
        //Debug.Log("Grid X: " + gridY);
    }
    ...
    private void TouchPressed(InputAction.CallbackContext context)
    {
        ...
        //Debug.Log("Going from: " + Mathf.Floor(player.transform.position.x) + "," + Mathf.Floor(player.transform.position.y));
        //Debug.Log("Going to: " + Mathf.Floor(worldpos.x) + "," + Mathf.Floor(worldpos.y));

        FindPath(new Vector2(Mathf.Floor(player.transform.position.x), Mathf.Floor(player.transform.position.y)), new Vector2(Mathf.Floor(worldpos.x), Mathf.Floor(worldpos.y)));
    }
    ...
    private void FindPath(Vector2 startPosition, Vector2 endPosition)
    {
        ...
        for(int x = 0; x < gridSize[0]; x++)
        {
            for(int y = 0; y < gridSize[1]; y++)
            {
                ...
                Debug.Log(floor.GetComponentInChildren<Tilemap>().GetTile((new Vector3Int(x, y, 0))));
